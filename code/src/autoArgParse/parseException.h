...
public:
    ...
    MissingMandatoryArgException(const ComplexFlag& complexFlag) :
            ParseException(MISSING_MANDATORY_ARG,
                    makeErrorMessage(complexFlag)), complexFlag(complexFlag) {
    }
...
public:
    ...
    MissingMandatoryFlagException(const ComplexFlag& complexFlag) :
            ParseException(MISSING_MANDATORY_FLAG,
                    makeErrorMessage(complexFlag)), complexFlag(complexFlag) {
    }
    static std::string makeErrorMessage(const ComplexFlag& complexFlag) {
        std::ostringstream os;
...
public:
    ...
    RepeatedFlagException(std::string repeatedFlag) :
            ParseException(REPEATED_FLAG, makeErrorMessage(repeatedFlag)), repeatedFlag(
                    std::move(repeatedFlag)) {
    }
...
public:
    ...
    UnexpectedArgException(std::string unexpectedArg,
            ...
            ParseException(UNEXPECTED_ARG,
                    makeErrorMessage(unexpectedArg, complexFlag)), unexpectedArg(
                    std::move(unexpectedArg)), complexFlag(complexFlag) {
    }
...
public:
    ...
    MoreThanOneExclusiveArgException(std::string exclusiveArg,
            ...
            ParseException(MORE_THAN_ONE_EXCLUSIVE_ARG,
                    makeErrorMessage(exclusiveArg, complexFlag)), exclusiveArg(
                    exclusiveArg), complexFlag(complexFlag) {
    }
    static std::string makeErrorMessage(const std::string& exclusiveArg,
            const ComplexFlag& complexFlag) {
        std::ostringstream os;
        os
                << "The following arguments are exclusive and may not be used in conjunction: ";
...
public:
    ...
    ConversionException(const std::string& argName,
            ...
            ParseException(CONVERSION_ERROR,
                    makeErrorMessage(argName, additionalExpl)), argName(
                    argName), additionalExpl(additionalExpl) {
    }
